...
class ServiceEquipo extends REST_Controller
{
    ...
    function user_post()
    {
        ...
        $data = array(
                   'var_Nombre' => $info['data']['nombre'],
                   'var_Abrev' => $info['data']['abrev']
                );
        switch ($info['data']['action']) {
            case 'add':
                ...
                break;
            case 'update':
                if($this->checkExist($info['data']['idEquipo'])){
                    $query = $this->db->update('tbl_Equipo', $data, array('int_Id_Equipo' => $info['data']['idEquipo'])); 
                }
                else{
    ...
    function user_delete()
    {
        ...
        $query = $this->db->delete('tbl_Equipo', array('int_Id_Equipo' => $info['data']->idEquipo)); 
        $this->response($query, 200); // 200 being the HTTP response code
    }
