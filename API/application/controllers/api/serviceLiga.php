...
class ServiceLiga extends REST_Controller
{
    ...
    function user_post()
    {
        ...
        switch ($info['data']['action']) {
            case 'add':
                ...
                break;
            case 'update':
                if($this->checkExist($info['data']['idLiga'])){
                    $query = $this->db->update('tbl_Liga', $data, array('int_Id_Liga' => $info['data']['idLiga'])); 
                }
                else{
    ...
    function user_delete()
    {
        ...
        $query = $this->db->delete('tbl_Liga', array('int_Id_Liga' => $info['data']->idLiga)); 
        $this->response($query, 200); // 200 being the HTTP response code
    }
