...
class ServicePuntaje extends REST_Controller
{
    ...
    function user_post()
    {
        ...
        switch ($info['data']['action']) {
            case 'add':
                ...
                break;
            case 'update':
                if($this->checkExist($info['data']['idPuntaje'])){
                    $query = $this->db->update('tbl_Puntaje', $data, array('int_Id_Puntaje' => $info['data']['idPuntaje'])); 
                }
                else{
    ...
    function user_delete()
    {
        ...
        $query = $this->db->delete('tbl_Puntaje', array('int_Id_Puntaje' => $info['data']->idPuntaje)); 
        $this->response($query, 200); // 200 being the HTTP response code
    }
